...
class ComplexBatchNorm(link.Link):
    ...
    def __init__(self, size, decay=0.9, eps=2e-5, dtype=np.float32,
        ...
        self.moving_Vrr = np.full(size/2, 1./np.sqrt(2), dtype=dtype)
        self.moving_Vii = np.full(size/2, 1./np.sqrt(2), dtype=dtype)
        self.moving_Vri = np.zeros(size/2, dtype=dtype)
        self.moving_real_mean = np.zeros(size/2, dtype=dtype)
        self.moving_imag_mean = np.zeros(size/2, dtype=dtype)


        self.register_persistent('moving_real_mean')
        self.register_persistent('moving_imag_mean')
        self.register_persistent('moving_Vii')
        self.register_persistent('moving_Vri')
        self.register_persistent('moving_Vrr')
    ...
    def __call__(self, x, **kwargs):
        ...
        if configuration.config.train:
            ...
            with chainer.no_backprop_mode(): 
                ...
                moving_real_mean = Variable(self.moving_real_mean)
                moving_imag_mean = Variable(self.moving_imag_mean)

                moving_real_mean = moving_real_mean*self.decay + mu_real*(1-self.decay)
                moving_imag_mean = moving_imag_mean*self.decay + mu_imag*(1-self.decay)
            mu_real = moving_real_mean
            mu_imag = moving_imag_mean
            self.moving_real_mean = moving_real_mean._data[0]
            self.moving_imag_mean = moving_imag_mean._data[0]
            ...
            #### Do the moving average stuff~~~~~  
            with chainer.no_backprop_mode(): 
                moving_Vrr =  Variable(self.moving_Vrr) 
                moving_Vri =  Variable(self.moving_Vri) 
                moving_Vii =  Variable(self.moving_Vii) 

                moving_Vrr = moving_Vrr*self.decay + Vrr*(1-self.decay)
                moving_Vii = moving_Vii*self.decay + Vii*(1-self.decay)
                moving_Vri = moving_Vri*self.decay + Vri*(1-self.decay)

            Vrr = moving_Vrr
            Vii = moving_Vii
            Vri = moving_Vri
            self.moving_Vrr = moving_Vrr._data[0]
            self.moving_Vri = moving_Vri._data[0]
            self.moving_Vii = moving_Vii._data[0]
